...
const schema = object({
  ...
  password: string()
    .min(8)
    .required()
    .matches(/^(?=.*[a-zA-Z])(?=.*[0-9])/, "Senha deve conter 8 caracteres e pelo menos uma letra e um número")
    .label("Senha"),
});
...
<template>
  ...
  <v-form class="mt-5">
    ...
    <VTextField
         v-model="firstName"
         :error-messages="errors.first_name"></VTextField>
    <v-label
      class="text-subtitle-1 font-weight-medium pb-2"
      >E-mail</v-label
    >
    <VTextField
     v-model="email"
     type="email"
     :error-messages="errors.email"></VTextField>
    <v-label
      class="text-subtitle-1 font-weight-medium pb-2"
      >Senha</v-label
    >
