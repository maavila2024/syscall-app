...
// Create a list of unique status options with "Todos"
...
// Handle status change to select/deselect all
...
// Watcher to update selectAll based on selectedStatuses
watch(selectedStatuses, (newValue) => {
  selectAll.value = newValue.length === statusOptionsWithAll.value.length - 1;
});

// Filter tasks based on the selected status
const filteredTasks = computed(() => {
  if (selectedStatuses.value.length === 0 || selectedStatuses.value.includes("Todos")) {
    return tasks.value;
  }
  return tasks.value.filter(task => selectedStatuses.value.includes(task.task_status.name));
});
