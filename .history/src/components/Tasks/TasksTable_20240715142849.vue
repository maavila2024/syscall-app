  ...
  <v-table class="border-opacity-100">
    ...
    <tbody>
      <tr v-for="(task, i) in tasks" :key="i" :style="getStatusStyle(task.task_status)">
        ...
        <td>
          <span class="limited-text">{{ task.name }}</span>
          <v-chip
            v-if="task.default"
            size="small"
            variant="tonal"
        ...
        <td><span class="limited-text">{{ task.description }}</span></td>
        <td><span class="limited-text">{{ task.user_owner.first_name }}</span></td>
        <td><span class="limited-text">{{ task.user_responsible?.first_name || "Nenhum responsável" }}</span></td>
        <td><span class="limited-text">{{ task.task_status.name }}</span></td>
          ...
          <div class="d-flex align-center">
            <v-tooltip :text="task.priority_justification">
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props">
                  <span v-bind="attrs" v-on="on" class="limited-text">
                    {{ task.priority.name }}
                  </span>
                </v-btn>
              </template>
...
.limited-text {
  display: -webkit-box;
  -webkit-line-clamp: 2; /* Limite para no máximo 2 linhas */
