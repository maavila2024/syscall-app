...
const { handleSubmit, errors, isSubmitting, values: form, setFieldValue, validate } = useForm({
  validationSchema: object({
    ...
    priority_justification: string().label("Justificativa da Prioridade"),
    status: boolean().required().label("Status"),
  }),
  initialValues,
  validateOnMount: false, // Não validar ao montar o componente
});
...
watch(() => form.priority_id, (newVal) => {
  ...
  if (!shouldShowPriorityJustification.value) {
    setFieldValue("priority_justification", "");
  }
});
</script>
