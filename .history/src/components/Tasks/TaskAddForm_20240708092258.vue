        ...
        <v-row>
          <v-col cols="12">
            <v-text-field
              ...
              label="Complexidade da Prioridade"
              variant="outlined"
              color="primary"
            />
          </v-col>
...
const defaultComplexity = computed(() => {
  return tasksStore.complexity.find(s => s.is_default)?.id || null;
});
...
const schema = computed(() => {
  return object({
    ...
    complexity_id: number().required().label('Prioridade'),
...
const { handleSubmit, errors, isSubmitting } = useForm({
  ...
  initialValues: {
    ...
    complexity_id: defaultComplexity.value,
    observation: '',
    created_at: '',
    expected_date: '',
    finish_date: '',
