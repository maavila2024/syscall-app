  ...
  <v-table class="border-opacity-100">
    ...
    <tbody>
      <tr v-for="(task, i) in tasks" :key="i" :style="getStatusStyle(task.task_status)">
        ...
        <td class="line-clamp-2">{{ task.name }}</td>
        ...
        <td class="line-clamp-1 text-right">{{ task.priority.name }}</td>
        <td class="line-clamp-1 text-right">{{ task.complexity?.name || "Aguardando análise" }}</td>
        <td class="text-right">
