    ...
    <v-table class="border-opacity-100">
      <thead>
        <tr>
          ...
          <th class="text-left" style="width: 100px;">Solicitante
            <v-btn icon @click="showOwnerFilter = !showOwnerFilter">
              <FilterIcon />
            </v-btn>
          </th>
          <th class="text-left" style="width: 100px;">Responsável
      ...
      <tbody>
        ...
        <tr v-for="(task, i) in filteredTasks" :key="i" >
          ...
          <td><div class="text-clamp" :style="getStatusStyle(task.task_status)">{{ task.task_status.name }}</div></td>
          <td class="text-right">
...
// Carregar tarefas com paginação
const loadTasks = async (page = 1) => {
  await tasksStore.getTasks({ page });
  currentPage.value = page;
  totalPages.value = Math.ceil(tasksStore.total / tasksPerPage);
};
...
// Chamando loadTasks na inicialização para carregar a primeira página
loadTasks();
