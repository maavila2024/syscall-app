...
const submit = handleSubmit(async (values) => {
  try {
    ...
    const task = await tasksStore.createInteraction(props.taskId, values.comment, props.userId);
  ...
  if (values.files && values.files.length > 0) {
      ...
      await tasksStore.uploadFile(formData);
    }
    await tasksStore.fetchMessages(props.taskId);
