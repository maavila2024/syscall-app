<template>
  <pre>{{ meStore.user.default_segment }}</pre>
  <div class="tasks-table-container">
    <div class="filter-container">
      <v-radio-group v-model="selectedSegment" @change="updateUserSegment">
...
const updateUserSegment = async () => {
  try {
    await axios.post(`api/users/update-segment`, { segment: selectedSegment.value });
    meStore.user.default_segment = selectedSegment.value;
    fetchTasks();
  } catch (error) {
...
onMounted(() => {
  fetchTasks();
  watch(() => route.query.search, fetchTasks);
});
