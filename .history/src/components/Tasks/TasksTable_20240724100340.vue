      ...
      <v-table class="border-opacity-100">
        <thead>
          <tr>
            ...
            <th class="text-left" style="width: 140px" @click="sortTable('description')">
              Descrição
        ...
        <tbody>
          ...
          <!-- Conteúdo da tabela -->
          <tr v-for="(task, i) in filteredTasks" :key="task.id">
            ...
            <td>
              <div class="text-clamp">{{ task?.user_owner?.first_name || '' }}</div> <!-- Adicione verificação de null aqui -->
            </td>
            <td>
              <div class="text-clamp">{{ task?.user_responsible?.first_name || "Nenhum responsável" }}</div>
            </td>
            <td>
              <div class="text-clamp" :style="getStatusStyle(task?.task_status)">{{ task?.task_status?.name || '' }}</div> <!-- Adicione verificação de null aqui -->
            </td>
            <td class="text-right">
                  ...
                  <template v-slot:activator="{ props }">
                    <v-btn v-bind="props">
                      <span v-bind="attrs" v-on="on">
                        <div class="text-clamp">{{ task?.priority?.name || '' }}</div> <!-- Adicione verificação de null aqui -->
                      </span>
                    </v-btn>
                  </template>
                </v-tooltip>
              ...
              <div class="d-flex align-center">
                <v-tooltip :text="task?.complexity_justification || 'Aguardando análise'">
                  <template v-slot:activator="{ props }">
                    <v-btn v-bind="props">
                      <span v-bind="attrs" v-on="on">
                        <div class="text-clamp">{{ task?.complexity?.name || "Aguardando análise" }}</div> <!-- Adicione verificação de null aqui -->
                      </span>
                    </v-btn>
                  </template>
                </v-tooltip>
...
const getStatusStyle = (taskStatus) => {
  return {
    color: taskStatus?.color || 'inherit',
