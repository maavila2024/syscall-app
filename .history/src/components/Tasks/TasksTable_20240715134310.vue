  ...
  <v-table class="border-opacity-100">
    ...
    <tbody>
      <tr v-for="(task, i) in tasks" :key="i" :style="getStatusStyle(task.task_status)">
        <td>{{ truncate(task.task_code, 30) }}</td>
        <td>
        ...
        <td>{{ truncate(task.user_owner.first_name, 30) }}</td>
        <td>{{ truncate(task.user_responsible?.first_name || "Nenhum responsável", 30) }}</td>
        <td>{{ truncate(task.task_status.name, 30) }}</td>
              ...
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props">
                  <span v-bind="attrs" v-on="on">
                    {{ truncate(task.priority.name, 30) }}
                  </span>
                </v-btn>
              </template>
            </v-tooltip>
          ...
          <div class="d-flex align-center">
            <v-tooltip :text="task.complexity_justification || 'Aguardando análise'">
              <template v-slot:activator="{ props }">
                <v-btn v-bind="props">
                  <span v-bind="attrs" v-on="on">
                    {{ truncate(task.complexity?.name || "Aguardando análise", 30) }}
                  </span>
                </v-btn>
              </template>
            </v-tooltip>
