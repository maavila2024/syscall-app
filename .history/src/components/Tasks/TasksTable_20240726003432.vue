...
// Função para buscar tarefas com base no task_code
const fetchTasks = (searchQuery) => {
  tasksStore.fetchTasks(searchQuery);
};

// Watcher para observar mudanças no parâmetro de consulta 'search'
watch(
  () => route.query.search,
  (newSearch) => {
    if (newSearch) {
      fetchTasks(newSearch);
    } else {
