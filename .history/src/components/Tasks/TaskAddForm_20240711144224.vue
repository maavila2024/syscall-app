...
const files = ref([]); // Novo campo para armazenar arquivos
...
const schema = object({
  ...
  files: array().of(mixed()).nullable(), // Adicionado array e mixed
});
...
const submit = handleSubmit(async (payload) => {
  try {
    const task = await tasksStore.storeTask(payload);
    ...
    if (files.value.length > 0) {
      ...
      await tasksStore.uploadTaskFile(formData);
    }
    emit("add");
  } catch (error) {
    console.error('Erro ao adicionar tarefa ou fazer upload dos arquivos:', error);
  }
});
