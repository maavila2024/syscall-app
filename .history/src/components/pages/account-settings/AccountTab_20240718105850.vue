<template>
  ...
  const schema = object({
    ...
    // newPassword: string().nullable().min(8).matches(/^(?=.*[a-zA-Z])(?=.*[0-9])/, "Password must contain 8 characters, at least one letter and one number").label("New Password"),
    // confirmPassword: string().nullable().oneOf([ref('newPassword'), null], 'Passwords must match').label('Confirm Password'),
  });
