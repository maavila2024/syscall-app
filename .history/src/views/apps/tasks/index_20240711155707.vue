<template>
  <div>
    ...
    <template v-else>
      <TasksTable @openChat="openChatModal" @openAttachments="openAttachmentsModal" />
    ...
    <v-dialog v-model="isAttachmentsDialogOpen" max-width="600px">
      <v-card>
        <v-card-title class="headline">Anexos do Chamado</v-card-title>
        <v-card-text>
          <v-list>
            <v-list-item v-for="file in taskFiles" :key="file.id">
              <v-list-item-content>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="isAttachmentsDialogOpen = false">Fechar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
import { TrashIcon, DeleteIcon } from 'vue-tabler-icons';
...
const { toEdit, toDelete, taskFiles } = storeToRefs(tasksStore);
...
const openAttachmentsModal = (task) => {
  tasksStore.getTaskFiles(task.id);
...
const deleteAttachment = async (fileId) => {
  ...
  tasksStore.getTaskFiles(selectedTaskId.value);
};
</script>