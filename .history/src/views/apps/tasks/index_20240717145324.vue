<template>
  <div>
    ...
    <template v-else>
      <TasksTable @openChat="openChatModal" @addNote="openAddNoteModal" @openAttachments="openAttachmentsModal" @viewTask="openViewModal"/>
      <v-dialog v-model="isShowing" width="auto">
        <template #default="{ isActive }">
          <v-card width="400">
            <v-card-title>Visualizar chamado</v-card-title>
            <v-card-text>
              <TaskShowForm :task="toShow" @cancel="isActive.value = false" />
            </v-card-text>
          </v-card>
        </template>
      </v-dialog>
