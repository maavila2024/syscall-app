...
const fetchStatistics = async () => {
  ...
  // Adicionando verificações para evitar erros
  const statusOpen = statistics?.statusOpen || 0;
  const statusInDevelopment = statistics?.statusInDevelopment || 0;
  const statusWaitingResponse = statistics?.statusWaitingResponse || 0;
  const statusSentForTesting = statistics?.statusSentForTesting || 0;
  const statusCompleted = statistics?.statusCompleted || 0;
  ...
  lineChart.value.series = [
    ...
    },
    {
      ...
      data: [statusInDevelopment],
    },
    {
      name: 'Aguardando resposta',
      data: [statusWaitingResponse],
    },
    {
      name: 'Enviado para teste',
      data: [statusSentForTesting],
    },
    {
      name: 'Concluído',
