...
console.log(moment().format("MMMM YYYY")); // Deve exibir o mês em português
...
const lineChartGrains = ref({
  series: [
    { name: "Total de Chamados", data: [], color: "#7E5B2A" },
...
const updateChartDataGrains = async (monthYear) => {
  try {
    ...
    lineChartGrains.value.series = [
      ...
      { name: "Aberto", data: [data.statusOpen || 0], color: "#253662" },
      {
      ...
      { name: "Concluído", data: [data.statusCompleted || 0], color: "#1B3C48" },
    ];
