...
const updateChartData = async (monthYear, segment) => {
  try {
    const data = await tasksStore.fetchTaskStatisticsBySegment(segment, monthYear);
...
watch(select, (newMonthYear) => {
  ...
  // updateChartData(newMonthYear, 2); // Para o segmento Proteína
});
...
onMounted(() => {
  ...
  // updateChartData(select.value, 2); // Carregar dados para o mês atual e segmento Proteína
});
</script>
